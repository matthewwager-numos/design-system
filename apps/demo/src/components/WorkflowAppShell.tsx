...
export interface WorkflowStubTab {
  /** Typically a lucide icon at size 24 — distinct from the app's own header icon. Omit for a text-only stub. */
  icon?: ReactNode;
    ...
    <>
      ...
      <div className="app-body">
        {TAB_IDS.map(
          (id) =>
            tab === id && (
              <EmptyState key={id} icon={tabs[id].icon} title={tabs[id].title ?? "Not built yet"} description={tabs[id].description} />
            ),
        )}
      </div>
    </>
