...
import { ACCRUAL_VENDORS } from "../../data/accruals";
...
export function HistoryTab() {
  ...
  const selectedVendor = ACCRUAL_VENDORS.find((vendor) => vendor.id === selectedVendorId) ?? null;
  ...
  const entries = useMemo(
    () =>
      ACCRUALS_HISTORY.map((entry) => ({
        ...entry,
        onSubjectClick: entry.subjectId ? () => setSelectedVendorId(entry.subjectId!) : undefined,
  ...
  return (
    <div className="page page--full-width page--fill-height">
      ...
      <VendorDetailDrawer vendor={selectedVendor} onClose={() => setSelectedVendorId(null)} />
